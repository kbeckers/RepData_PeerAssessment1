...
The mean total number of steps taken per day is `r mean`and is depicted in blue.
The median total number of steps taken per day is `r median` and is depicted in red.  
...
the maximum number of steps is `r intervalmoststeps`.
...
There are `r numna` records with missing values in the dataset, which means that no data was 
recorded during roughly `r daysmissing` of days.
...
```{r replace missing values by mean values, echo = TRUE}
...
It has increased by `r daysmissing` which is logical, as there were `r daysmissing` days where the data were missing. As 
